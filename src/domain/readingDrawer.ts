...
export const readingDrawerCategories: {
  ...
  },
  {
    id: "danger_safety",
    label: "危険牌・安全度",
    description: "筋、壁、現物など危険牌比較で残しやすい候補。",
  },
  {
    id: "score_threshold",
    label: "点数状況",
    description: "順位点、オーラス条件、供託、本場で行動閾値を動かす。",
  },
  {
    id: "table_dynamics",
    label: "卓上動態",
...
export const readingDrawerItems: ReadingDrawerItem[] = [
  item("call_intent", "速度副露", "和了到達やテンパイ率を上げるための鳴き。", 0.08, [
    progress("+", 0.18),
  ]),
  item("call_intent", "役牌バック", "役牌後付けや役牌重なりを見た副露。", 0.07, [
    value("+", 0.12),
    wait("mixed", 0.08),
  ]),
  item("call_intent", "染め移行", "色寄せの途中段階。打点上昇と速度低下が同時に起きやすい。", 0.1, [
    value("+", 0.22),
    progress("mixed", 0.1),
  ]),
  item("call_intent", "形式テンパイ", "終盤のノーテン罰符回避や形式聴牌狙い。", 0.06, [
    progress("+", 0.12),
    value("-", 0.08),
  ]),
  item("call_intent", "安牌確保副露", "手牌進行よりも安全牌や守備余地を確保する鳴き。", 0.05, [
    wait("mixed", 0.08),
  ]),
  item("call_intent", "牽制/ブラフ", "本線確率を上げすぎないために残す牽制・ブラフ候補。", 0.04, [
    value("unknown", 0.05),
  ], "hard prune前に相手傾向と河の一貫性を見る。"),

  item("value_pattern", "ドラ絡み", "ドラ周辺やドラ色から打点尾部を残す。", 0.08, [
    value("+", 0.2),
  ]),
  ...
  item("value_pattern", "役牌重なり", "役牌対子/暗刻からの打点・速度候補。", 0.07, [
    value("+", 0.12),
    progress("+", 0.1),
  ]),
  item("value_pattern", "親番高打点", "親の連荘価値込みで押し閾値が下がる候補。", 0.06, [
    value("+", 0.16),
    threshold("+", 0.12),
  ]),

  item("progress_pattern", "先制テンパイ", "先にテンパイしている読みを残す。", 0.09, [
    progress("+", 0.22),
  ]),
  item("progress_pattern", "1シャンテン", "テンパイ手前で押し返し余地がある候補。", 0.08, [
    progress("+", 0.14),
  ]),
  item("progress_pattern", "まだ遠い", "進行度が低く、打点や守備意図を別に見る候補。", 0.06, [
    progress("-", 0.16),
  ]),
  item("progress_pattern", "鳴きで急加速", "鳴き後にテンパイ率が急に上がる形。", 0.08, [
    progress("+", 0.2),
  ]),
  item("progress_pattern", "巡目に対して遅い", "巡目との比較で本線化しすぎを抑える候補。", 0.05, [
    progress("-", 0.14),
  ]),

  item("wait_shape", "良形テンパイ", "両面以上のテンパイとして残す候補。", 0.07, [
    wait("+", 0.18),
  ]),
  item("wait_shape", "愚形テンパイ", "愚形で和了率や押し返しが落ちる候補。", 0.07, [
    wait("-", 0.18),
  ]),
  ...
  item("wait_shape", "変化残り", "現在形だけでなく次巡以降の改善を残す。", 0.05, [
    wait("+", 0.1),
  ]),
  item("wait_shape", "待ち候補不明", "観測不足で待ち形を断定しないための未知候補。", 0.08, [
    wait("unknown", 0.16),
  ], "安全度評価を確定しすぎない。"),
  ...
  item("danger_safety", "片筋", "安全とも危険とも決めきれない比較候補。", 0.04, [
    wait("mixed", 0.08),
  ]),
  item("danger_safety", "壁", "壁情報による安全度上昇候補。", 0.04, [
    wait("+", 0.08),
  ]),
  item("danger_safety", "ワンチャンス", "安全度を少し上げるが過信しない候補。", 0.04, [
    wait("mixed", 0.08),
  ]),
  item("danger_safety", "現物", "守備寄せや押し引き閾値を変える安全候補。", 0.05, [
    threshold("+", 0.08),
  ]),
  item("danger_safety", "スジ引っかけ", "筋を安全扱いしすぎない例外候補。", 0.04, [
    wait("unknown", 0.08),
  ]),
  ...
  item("score_threshold", "トップ目維持", "リード維持で押し閾値が上がる局面。", 0.05, [
    threshold("-", 0.14),
  ]),
  item("score_threshold", "ラス回避", "放銃回避や着順価値で閾値が変わる局面。", 0.06, [
    threshold("mixed", 0.16),
  ]),
  item("score_threshold", "親番維持", "親番の連荘価値で押し寄りになる候補。", 0.06, [
    threshold("+", 0.14),
  ]),
  item("score_threshold", "オーラス条件", "着順条件で必要打点や押し引きが固定される候補。", 0.06, [
    threshold("mixed", 0.18),
  ]),
  item("score_threshold", "供託回収", "供託により和了価値が上がる候補。", 0.04, [
    threshold("+", 0.08),
  ]),
  item("score_threshold", "本場価値", "本場込みの押し返し価値を残す候補。", 0.04, [
    threshold("+", 0.08),
  ]),

  item("table_dynamics", "脇救済", "脇の和了/放銃で自分の不利が緩む候補。", 0.06, [
    threshold("mixed", 0.14),
  ]),
  item("table_dynamics", "脇の和了", "対象以外の和了で局面が終了する可能性。", 0.05, [
    threshold("mixed", 0.1),
  ]),
  item("table_dynamics", "脇の放銃", "対象以外の放銃で点数状況が変わる候補。", 0.04, [
    threshold("mixed", 0.1),
  ]),
  item("table_dynamics", "鳴きによる安牌供給", "他家の鳴きで安全牌が増える候補。", 0.04, [
    wait("+", 0.07),
  ]),
  item("table_dynamics", "流局接近", "流局までの距離でテンパイ料や安全度が変わる候補。", 0.05, [
    progress("mixed", 0.08),
    threshold("mixed", 0.08),
  ]),

  item("player_tendency", "鳴きが軽い", "軽い仕掛けを高打点や本線に寄せすぎない。", 0.06, [
    progress("+", 0.1),
    value("-", 0.08),
  ]),
  item("player_tendency", "打点寄せ", "速度より打点を重視する相手傾向。", 0.06, [
    value("+", 0.14),
  ]),
  item("player_tendency", "守備寄せ", "攻撃本線の過大評価を抑える傾向。", 0.05, [
    progress("-", 0.1),
  ]),
  item("player_tendency", "ブラフ多め", "観測と打牌意図のズレを残す傾向。", 0.05, [
    value("unknown", 0.08),
  ]),
  item("player_tendency", "押し返し多め", "押し返し発生で安全度や進行度が変わる候補。", 0.05, [
    progress("+", 0.09),
    threshold("+", 0.08),
  ]),

  item("exception_noise", "空切り", "手出し/ツモ切り読みを崩す例外。", 0.04, [
    wait("unknown", 0.07),
  ], "観測ミスと区別して低確率で残す。"),
  item("exception_noise", "手順偽装", "手順から本線を断定しすぎないための例外。", 0.04, [
    value("unknown", 0.07),
  ]),
  item("exception_noise", "観測ミス", "見落としや記録漏れを未知バッファに残す。", 0.05, [
    wait("unknown", 0.08),
  ]),
  item("exception_noise", "記憶違い", "局面再現の不確実性を表す候補。", 0.04, [
    progress("unknown", 0.06),
  ]),
  item("exception_noise", "河のノイズ", "河からの意図推定を弱めるノイズ。", 0.05, [
    wait("mixed", 0.07),
  ]),
  item("exception_noise", "レア役", "低頻度だが枝刈り前に残す役候補。", 0.03, [
    value("+", 0.09),
  ]),
  item("exception_noise", "意図不明打牌", "説明不能な打牌を無理に既存候補へ吸収しない。", 0.05, [
    value("unknown", 0.06),
    wait("unknown", 0.06),
  ], "例外集か未知バッファとして保存する。"),
];
...
export function createChoiceCandidateFromDrawerItem(
  ...
  const assigned = clampProbability(probability ?? item.default_probability ?? 0);
  return {
    label: item.label,
...
export function createResidualBucketFromDrawerItem(
  ...
  return {
    ...
    label: item.label,
    kind: item.category === "exception_noise" ? "exception" : "unrecalled_candidate",
...
function item(
  ...
  return {
    ...
    tags: unique([category, label, ...axisImpacts.map((impact) => impact.axis_id)]),
    caution,
  };
}
