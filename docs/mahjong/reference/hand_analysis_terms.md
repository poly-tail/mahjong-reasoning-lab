...
- 生の残筋本数:
  - 固定18筋線へ suppressor だけを適用した `raw_count` (`0 / 1`) の合計。
  - 係数適用後の panel `Remain` や放銃確率とは区別する。

- 補正後筋質量:
  - またぎ筋、見え枚数、鳴き形、内外、裏筋、長考ツモ切り、lag の名前付き係数を既定順で適用した筋線 weight の合計。
  - 現行 panel の `Remain` はこの float 値であり、生の本数や校正済み確率ではない。

- 筋危険度 score:
  - 候補牌に属する補正後筋質量を、その局面の補正後筋質量合計で割った heuristic score。
  - 愚形加算や聴牌近さ係数は別段階で適用し、統計的な放銃確率とは呼ばない。
