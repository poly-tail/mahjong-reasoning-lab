...
候補木ビューは、内部グラフ/DAGを壊さず、読み候補空間を木構造風に投影する表示モードです。ユーザーは候補確率、未配分確率、4軸影響、例外、未知、未展開候補を枝として見ながら整理します。候補木ビューは押し引き判断や牌選択ではなく、読み候補空間の整理UIです。

表示ラベルは日本語の枝操作に統一します。候補を除外する操作は `枝を切る`、重みを下げる操作は `枝を弱める`、上位候補を残す操作は `有力枝を残す`、固定系操作は `枝を固定する`、`比率を固定する` と表示します。未配分確率は `未展開の枝`、例外候補は `例外の枝置き場`、候補追加元は `読みの枝候補` と表示します。

候補木ビューの画面は、左に候補グループと枝、中央に選択中の枝詳細、右に枝操作、下に反映前確認を置きます。反映前確認では操作前後の候補確率、影響、警告を表示し、`反映前確認`、`反映する`、`元に戻す` を表示します。

候補木ビューは Sheet / Project / Workspace の表示スコープに対応します。新規ProjectまたはSheet作成時にテンプレートを配置した場合、`牌理`、`枚数`、`手役`、`抽象的な読み` は初期枝として見える必要があります。

枝を切る操作は危険操作として扱います。mixed / unknown の軸、未展開の枝、未知の枝、例外の枝置き場、軸確信度が低い大きな影響、固定中の枝がある場合は、反映前確認で警告を出します。

現行受け入れ範囲は、候補木への投影、枝選択、操作ラベル選択、警告プレビューまでです。候補木内の `反映前確認` / `反映する` / `元に戻す`、未展開・例外への送信、読みの枝候補から追加するボタンはworkspaceを変更しません。永続データの確率編集、伝播preview適用、枝刈り記録は既存の詳細編集またはReasoning Labを使います。候補木から同じstore actionへ接続することは将来要件です。
...
現行Phaseでは、牌理読み、枚数読み、手役読み、抽象的な読みを、それぞれ別の候補木・別の100%空間として保持する。各候補木の入力確率、未配分確率、例外集、未知バッファ、4軸影響スコアを混同しない。

将来的に観点横断の補正を行う場合、ある観点の読みが別観点の候補分布を大きく変えることがある。たとえば抽象的な読みの影響で手役Aが薄くなり、手役Bが濃くなる場合でも、薄くなった候補の確率質量を常に手役Bへ全量再配分できるとは限らない。

特に、既存候補の説明力が低い、有力候補が少数しかない、未解決の mixed / unknown が残っている、部分的な牌姿パターン照合が不足している、抽象読みは濃いが対応する具体的な牌理・手役候補がまだ定義されていない、候補Aが薄くなったが候補Bへ寄せる根拠も十分ではない、例外候補として保持すべき低頻度・高損失パターンがある、という場合は、未展開の枝、例外の枝置き場、未知バッファの確率を増やす必要がある可能性がある。

Phase1では、この再配分ロジックを実装しない。未展開・例外確率を横断補正で自動増減させない。横断補正後の分布を入力確率へ上書きしない。抽象読みだけで既存候補を自動的に大きく切らない。牌姿パターン照合なしで横断統合を確定値として扱わない。4軸影響スコアを候補確率100%空間に混ぜない。

将来仕様としては、横断補正後に未展開・例外・未知バッファの確率を増減させる仕組み、既存候補へ再配分するか未展開・例外へ逃がすかを選ぶUI、有力候補数・説明力・軸確信度・mixed / unknown・未配分率に応じた再配分ルール、既存候補へ寄せるのか未展開・例外へ戻すのかを区別するプレビュー、横断補正ルールごとの信頼度と影響ログ、部分的な牌姿パターン照合による補正妥当性の検証を検討する。

統合後分布を扱う場合でも、入力確率、枚数補正後重み、横断補正後重み、計算用確率、未展開・例外確率は分離して表示・保存する方針とする。
...
- Residual Massは未配分候補の送信先として active Sheet / Project / Global / unknown buffer を表示上選べる。
- 現行の送信先選択はpanel内のUI状態であり、追加・未知保持handlerへ送信先を渡さない。新規要素の永続所属はactive Sheetとし、Project / Globalへの実ルーティングは将来要件とする。
...
- 現行文書の入口は `docs/README.md` とする。
- 要件の正本は本書、実装契約は `detailed-specification.md`、画面構成は `screen-specification.md`、利用者向け挙動は `specification.md` とする。
- schema変更時は `schema.md`、Project / Sheetや永続化変更時は `architecture.md` を同時に更新する。
- UIにボタンが存在しても永続データを変更しない場合は、実行可能な機能として記載しない。
- 要件定義、詳細仕様、ユーザー向け仕様、使い方ガイドのMarkdown変更時は対応PDFを再生成する。
